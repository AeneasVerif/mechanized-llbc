...
Proof.
  intros. destruct chunk; simpl; try apply Z.divide_refl; try (exists 2; lia); try (exists 4; lia); try (exists 8; lia).
Qed.
...
Definition encode_val (chunk: memory_chunk) (v: val) : list memval :=
  ...
  | Vint n, (Mint32 | Mint32al1) => inj_bytes (encode_int 4%nat (Int.unsigned n))
  | Vptr b ofs, Mint32 => if Archi.ptr64 then List.repeat Undef 4%nat else inj_value Q32 v
  | Vptr b ofs, Mint32al1 => if Archi.ptr64 then List.repeat Undef 4%nat else inj_value Q32 v
  | Vlong n, (Mint64 | Mint64al1) => inj_bytes (encode_int 8%nat (Int64.unsigned n))
  | Vptr b ofs, Mint64 => if Archi.ptr64 then inj_value Q64 v else List.repeat Undef 8%nat
  | Vptr b ofs, Mint64al1 => if Archi.ptr64 then inj_value Q64 v else List.repeat Undef 8%nat
...
Definition decode_val (chunk: memory_chunk) (vl: list memval) : val :=
  ...
  | Some bl =>
      ...
      | Mint32 | Mint32al1 => Vint(Int.repr(decode_int bl))
      | Mint64 | Mint64al1 => Vlong(Int64.repr(decode_int bl))
  ...
  | None =>
      match chunk with
      | Mint32 | Mint32al1 => if Archi.ptr64 then Vundef else Val.load_result chunk (proj_value Q32 vl)
      | Many32 => Val.load_result chunk (proj_value Q32 vl)
      | Mint64 | Mint64al1 => if Archi.ptr64 then Val.load_result chunk (proj_value Q64 vl) else Vundef
...
Definition decode_encode_val (v1: val) (chunk1 chunk2: memory_chunk) (v2: val) : Prop :=
  ...
  | Vint n, Mint32al1, Mint32al1 => v2 = Vint n
  | Vint n, Mint32, Mint32al1 => v2 = Vint n
  | Vint n, Mint32al1, Mint32 => v2 = Vint n
  | Vint n, Many32, Many32 => v2 = Vint n
  | Vint n, Mint32, Mfloat32 => v2 = Vsingle(Float32.of_bits n)
  | Vint n, Many64, Many64 => v2 = Vint n
  | Vint n, (Mint64 | Mint64al1 | Mfloat32 | Mfloat64 | Many64), _ => v2 = Vundef
  | Vint n, _, _ => True (**r nothing meaningful to say about v2 *)
  | Vptr b ofs, (Mint32 | Mint32al1 | Many32), (Mint32 | Mint32al1 | Many32) => v2 = if Archi.ptr64 then Vundef else Vptr b ofs
  | Vptr b ofs, (Mint64 | Mint64al1), (Mint64 | Mint64al1 | Many64) => v2 = if Archi.ptr64 then Vptr b ofs else Vundef
  | Vptr b ofs, Many64, Many64 => v2 = Vptr b ofs
  | Vptr b ofs, Many64, (Mint64 | Mint64al1) => v2 = if Archi.ptr64 then Vptr b ofs else Vundef
  | Vptr b ofs, _, _ => v2 = Vundef
  | Vlong n, Mint64, Mint64 => v2 = Vlong n
  | Vlong n, Mint64al1, Mint64al1 => v2 = Vlong n
  | Vlong n, Mint64, Mint64al1 => v2 = Vlong n
  | Vlong n, Mint64al1, Mint64 => v2 = Vlong n
  | Vlong n, Mint64, Mfloat64 => v2 = Vfloat(Float.of_bits n)
  | Vlong n, Mint64al1, Mfloat64 => v2 = Vfloat(Float.of_bits n)
  | Vlong n, Many64, Many64 => v2 = Vlong n
  | Vlong n, (Mbool|Mint8signed|Mint8unsigned|Mint16signed|Mint16unsigned|Mint32|Mint32al1|Mfloat32|Mfloat64|Many32), _ => v2 = Vundef
  ...
  | Vfloat f, Mfloat64, Mint64al1 => v2 = Vlong(Float.to_bits f)
  | Vfloat f, Many64, Many64 => v2 = Vfloat f
  | Vfloat f, (Mbool|Mint8signed|Mint8unsigned|Mint16signed|Mint16unsigned|Mint32|Mint32al1|Mfloat32|Mint64|Mint64al1|Many32), _ => v2 = Vundef
  ...
  | Vsingle f, (Mbool|Mint8signed|Mint8unsigned|Mint16signed|Mint16unsigned|Mint32|Mint32al1|Mint64|Mint64al1|Mfloat64|Many64), _ => v2 = Vundef
...
Opaque inj_value.
  intros.
  destruct v; destruct chunk1; try (simpl; apply decode_val_undef);
  destruct chunk2; unfold decode_encode_val, decode_val, encode_val, Val.load_result;
  repeat solve_decode_encode_val_general;
  try (rewrite Float.of_to_bits; auto);
  try (rewrite Float32.of_to_bits; auto).
Qed.
...
Definition quantity_chunk (chunk: memory_chunk) :=
  match chunk with
  | Mint64 | Mint64al1 | Mfloat64 | Many64 => Q64
  | _ => Q32
  end.

Inductive shape_encoding (chunk: memory_chunk) (v: val): list memval -> Prop :=
  | shape_encoding_f: forall q i mvl,
      (chunk = Mint32 \/ chunk = Mint32al1 \/ chunk = Many32 \/ chunk = Mint64 \/ chunk = Mint64al1 \/ chunk = Many64) ->
...
Proof.
             ...
             shape_decoding chunk (mv1 :: mvl) (Val.load_result chunk (proj_value q (mv1 :: mvl)))).
  {
    ...
    destruct H0 as [E|[E|[E|[E|[E|E]]]]]; subst chunk; destruct q; auto || discriminate.
    congruence.
  ...
  destruct chunk, Archi.ptr64; (apply shape_decoding_u || apply C); auto 7.
Qed.
...
(** On little-endian, assembling 4 bytes with shifts and ORs yields the
    same result as [int_of_bytes]. *)

Lemma int_of_bytes_4:
  forall b0 b1 b2 b3,
  int_of_bytes (b0 :: b1 :: b2 :: b3 :: nil) =
  Byte.unsigned b0 + Byte.unsigned b1 * 256 +
  Byte.unsigned b2 * 256 * 256 + Byte.unsigned b3 * 256 * 256 * 256.
Proof.
  intros. simpl. lia.
Qed.

Lemma int_of_bytes_8:
  forall b0 b1 b2 b3 b4 b5 b6 b7,
  int_of_bytes (b0 :: b1 :: b2 :: b3 :: b4 :: b5 :: b6 :: b7 :: nil) =
  Byte.unsigned b0
  + Byte.unsigned b1 * 256
  + Byte.unsigned b2 * 256 * 256
  + Byte.unsigned b3 * 256 * 256 * 256
  + Byte.unsigned b4 * 256 * 256 * 256 * 256
  + Byte.unsigned b5 * 256 * 256 * 256 * 256 * 256
  + Byte.unsigned b6 * 256 * 256 * 256 * 256 * 256 * 256
  + Byte.unsigned b7 * 256 * 256 * 256 * 256 * 256 * 256 * 256.
Proof.
  intros. simpl. lia.
Qed.

(** Decoding consecutive [Byte] memvals into a Mint32(al1) word, on
    little-endian targets. *)

Lemma decode_val_Mint32_assembly_le:
  forall b0 b1 b2 b3,
  Archi.big_endian = false ->
  decode_val Mint32 (Byte b0 :: Byte b1 :: Byte b2 :: Byte b3 :: nil) =
  Vint (Int.repr (Byte.unsigned b0
                + Byte.unsigned b1 * 256
                + Byte.unsigned b2 * 256 * 256
                + Byte.unsigned b3 * 256 * 256 * 256)).
Proof.
  intros. unfold decode_val. simpl proj_bytes.
  unfold decode_int, rev_if_be. rewrite H. f_equal. f_equal.
  apply int_of_bytes_4.
Qed.

Lemma decode_val_Mint32al1_assembly_le:
  forall b0 b1 b2 b3,
  Archi.big_endian = false ->
  decode_val Mint32al1 (Byte b0 :: Byte b1 :: Byte b2 :: Byte b3 :: nil) =
  Vint (Int.repr (Byte.unsigned b0
                + Byte.unsigned b1 * 256
                + Byte.unsigned b2 * 256 * 256
                + Byte.unsigned b3 * 256 * 256 * 256)).
...
(** Decoding consecutive [Byte] memvals into a Mint64(al1) long, on
    little-endian targets. *)

Lemma decode_val_Mint64_assembly_le:
  forall b0 b1 b2 b3 b4 b5 b6 b7,
  Archi.big_endian = false ->
  decode_val Mint64
    (Byte b0 :: Byte b1 :: Byte b2 :: Byte b3 ::
     Byte b4 :: Byte b5 :: Byte b6 :: Byte b7 :: nil) =
  Vlong (Int64.repr
    (Byte.unsigned b0
    + Byte.unsigned b1 * 256
    + Byte.unsigned b2 * 256 * 256
...
Proof.
  intros. unfold decode_val. simpl proj_bytes.
  unfold decode_int, rev_if_be. rewrite H. f_equal. f_equal.
  apply int_of_bytes_8.
Qed.

Lemma decode_val_Mint64al1_assembly_le:
  forall b0 b1 b2 b3 b4 b5 b6 b7,
  Archi.big_endian = false ->
  decode_val Mint64al1
    (Byte b0 :: Byte b1 :: Byte b2 :: Byte b3 ::
     Byte b4 :: Byte b5 :: Byte b6 :: Byte b7 :: nil) =
  Vlong (Int64.repr
    (Byte.unsigned b0
