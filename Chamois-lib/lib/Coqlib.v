...
From Stdlib Require Export String ZArith Znumtheory List Bool Lia.
...
(** Solves a goal [forall x0 x1, H x0 x1 -> P x0 <-> P x1] by generating a goal
    [forall x0 x1, H x0 x1 -> P x0 -> P x1] and using symmetry on [H] for the reverse direction. *)
Ltac symmetric_iff :=
  let H  := fresh "H"  in
  eassert _ as H; [|
    let Hs := fresh "Hs" in
...
(** For [H : forall (x : A), Q x], generate a subgoal [x : A] and change [H] into [Q x] *)
Ltac forward1 H :=
  refine (modusponens _ _ (H _) _);
  [|clear H; intro H].
...
(** Lifting a predicate to an option type. *)

Definition if_Some [A] (o : option A) (P : A -> Prop) : Prop :=
  match o with
  | Some x => P x
  | None   => True
  end.

Lemma if_Some_iff [A] (o : option A) (P : A -> Prop):
  if_Some o P <-> forall (x : A), o = Some x -> P x.
Proof.
  case o as [x|]; simpl; split; intuition solve [auto | congruence].
Qed.

Lemma if_Some_morph [A] (o : option A) P0 P1
  (P_EQ : forall x, P0 x <-> P1 x):
  if_Some o P0 <-> if_Some o P1.
Proof.
  case o; simpl; tauto.
Qed.

Definition ex_Some [A : Type] (o : option A) (P : A -> Prop) : Prop :=
  match o with
  | Some x => P x
  | None   => False
  end.

Lemma ex_Some_iff [A] (o : option A) (P : A -> Prop):
  ex_Some o P <-> exists (x : A), o = Some x /\ P x.
Proof.
  case o as [x|]; simpl; (split; [|intros (? & ? & ?)]); intuition solve [eauto | congruence].
Qed.
...
Lemma fold_left_ext : forall A B (f g : A -> B -> A) xs b,
    (forall a b, f a b = g a b) ->
    fold_left f xs b = fold_left g xs b.
Proof.
  induction xs; intros * EQ; simpl; auto.
  rewrite EQ, IHxs; auto.
Qed.

Lemma fold_left_map : forall A B C (f: A -> B) (g : C -> B -> C) xs c,
    fold_left g (List.map f xs) c = fold_left (fun c a => g c (f a)) xs c.
Proof.
...
(** [tr_map f l] computes [map f l] using only tail-recursive functions,
    avoiding stack overflows on very large lists. *)

Fixpoint rev_map {A B: Type} (f: A -> B) (l: list A) (acc: list B) : list B :=
  match l with
  | nil => acc
  | x :: l' => rev_map f l' (f x :: acc)
  end.

Definition tr_map {A B: Type} (f: A -> B) (l: list A) : list B :=
  rev_append (rev_map f l nil) nil.

Lemma rev_map_app: forall {A B} (f: A -> B) l acc,
  rev_map f l acc = rev (map f l) ++ acc.
Proof.
  induction l; simpl; intros.
  - auto.
  - rewrite IHl. rewrite <- app_assoc. simpl. auto.
Qed.

Lemma tr_map_eq: forall {A B} (f: A -> B) l,
  tr_map f l = map f l.
Proof.
  unfold tr_map. intros.
  rewrite rev_append_rev. rewrite app_nil_r.
  rewrite rev_map_app. simpl. rewrite app_nil_r.
...
Qed.
#[global] Hint Resolve is_tail_app: coqlib.
...
Lemma list_forall2_cons_iff x xs y ys:
  list_forall2 (x :: xs) (y :: ys) <-> P x y /\ list_forall2 xs ys.
Proof.
  split; [inversion 1 | constructor]; tauto.
Qed.
...
Lemma list_forall2_map1 [A0 A1 B] (f : A0 -> A1) (P : A1 -> B -> Prop) (u : list A0) (v : list B):
  list_forall2 P (map f u) v <-> list_forall2 (fun x => P (f x)) u v.
Proof.
  revert v; induction u as [|x xs IH]; intros [|]; try solve [split; inversion 1]; simpl.
  - split; constructor.
  - rewrite !list_forall2_cons_iff, IH; reflexivity.
Qed.

Lemma list_forall2_map2 [A B0 B1] (f : B0 -> B1) (P : A -> B1 -> Prop) (u : list A) (v : list B0):
  list_forall2 P u (map f v) <-> list_forall2 (fun x y => P x (f y)) u v.
Proof.
  revert v; induction u as [|x xs IH]; intros [|]; try solve [split; inversion 1]; simpl.
  - split; constructor.
  - rewrite !list_forall2_cons_iff, IH; reflexivity.
...
Lemma list_forall2_map : forall A1 A2 B1 B2 (f1: A1 -> B1) (f2: A2 -> B2) P l1 l2,
    list_forall2 (fun x1 x2 => P (f1 x1) (f2 x2)) l1 l2 ->
    list_forall2 P (List.map f1 l1) (List.map f2 l2).
Proof.
  intros * F2; induction F2; simpl; constructor; auto.
Qed.

Lemma list_forall2_same {A} : forall (R: A -> A -> Prop) l,
    (forall x, In x l -> R x x) ->
    list_forall2 R l l.
Proof.
  induction l; constructor; simpl in *; eauto.
Qed.

Lemma list_forall2_swap [A B] (P : A -> B -> Prop) u v:
  list_forall2 P u v <-> list_forall2 (fun y x => P x y) v u.
Proof.
  revert v; induction u as [|x xs IH]; intros [|]; try solve [split; inversion 1]; simpl.
  - split; constructor.
  - rewrite !list_forall2_cons_iff, IH; reflexivity.
...
Lemma proj_sumbool_false [P Q : Prop] (a : {P} + {Q}):
  proj_sumbool a = false -> Q.
Proof.
  destruct a; auto; discriminate 1.
Qed.
...
Lemma proj_sumbool_is_false [P : Prop] (a : {P} + {~ P}) (H : ~P):
