...
(*  published by the Free Software Foundation, either vesion 2.1 of   *)
...
From Stdlib Require Import Reals Program.
...
Definition to_single_UP: float -> float32 := Bconv _ _ 24 128 __ __ to_single_nan mode_UP.
Definition to_single_DN: float -> float32 := Bconv _ _ 24 128 __ __ to_single_nan mode_DN.
...
Theorem cmp_ne_nan:
  forall f, cmp Cne f f = (is_nan _ _ f).
Proof.
  intros f.
  destruct f; simpl; auto.
  destruct s; auto.
  replace (e ?= e) with Eq by (symmetry; apply Z.compare_eq_iff; reflexivity).
  replace (Pos.compare_cont Eq m m) with Eq by (rewrite Pos.compare_cont_refl; reflexivity).
...
Theorem ge_single_float: forall x y,
    Float.cmp Cge (Float.of_single x) y =
      Float32.cmp Cge x (Float.to_single_UP y).
Proof.
  intros. unfold Float.of_single, Float.to_single_UP. simpl.
  unfold Float.compare, Float32.compare.
  apply Bcompare_Bconv_widen_ge; lia.
Qed.

Remark cmp_lt_is_neg_ge: forall cmp,
  match cmp
  with
  | Some Lt => true
  | _ => false
  end = negb (cmp_ge_nan true cmp).
Proof.
  destruct cmp. 2: reflexivity.
  destruct c; reflexivity.
Qed.
  
Theorem lt_single_float: forall x y,
    Float.cmp Clt (Float.of_single x) y =
      Float32.cmp Clt x (Float.to_single_UP y).
Proof.
  intros. unfold Float.of_single, Float.to_single_UP. simpl.
  repeat rewrite cmp_lt_is_neg_ge.
  f_equal.
  apply Bcompare_Bconv_widen_ge; lia.
Qed.

Remark pseudo_nan64:
   {x : binary_float 53 1024 | is_nan 53 1024 x = true}.
Proof.
  exists (B754_nan 53 1024 false 1 (eq_refl _)).
  reflexivity.
Qed.

Remark pseudo_nan32:
   {x : binary_float 24 128 | is_nan 24 128 x = true}.
Proof.
  exists (B754_nan 24 128 false 1 (eq_refl _)).
  reflexivity.
Qed.

Lemma cmp_le_expand: forall cmp,
    match cmp 
  with
  | Some Eq | Some Lt => true
  | _ => false
    end = cmp_ge_nan false (option_map CompOpp cmp).
Proof.
  destruct cmp. 2: reflexivity. destruct c; reflexivity.
Qed.

(*
Lemma Bcompare_nan_l: forall prec emax x y (NAN: is_nan _ _ x = true),
    Bcompare prec emax x y = None.
Proof.
  intros.
  destruct x; try discriminate. reflexivity.
Qed.

Lemma Bcompare_nan_r: forall prec emax x y (NAN: is_nan _ _ y = true),
    Bcompare prec emax x y = None.
Proof.
  intros. rewrite Bcompare_swap.
  destruct y; try discriminate. reflexivity.
Qed.
 *)

Lemma option_map_CompOpp_invol:
  forall x, option_map CompOpp (option_map CompOpp x) = x.
Proof.
  destruct x. 2: reflexivity. destruct c; reflexivity.
Qed.

Theorem le_single_float: forall x y,
    Float.cmp Cle (Float.of_single x) y =
      Float32.cmp Cle x (Float.to_single_DN y).
Proof.
  intros. unfold Float.of_single, Float.to_single_DN. simpl.
  unfold Float.compare, Float32.compare.
  destruct (is_nan _ _ y || is_nan _ _ x) eqn:NAN.
  { rewrite orb_true_iff in NAN.
    destruct NAN.
    - rewrite (Bcompare_swap _ _ y).
      rewrite (Bcompare_swap _ _ _ x).
      destruct y; try discriminate. reflexivity.
    - destruct x; try discriminate. reflexivity.
  }
  rewrite orb_false_iff in NAN.
  destruct NAN as (NAN_y & NAN_x).
  rewrite (Bcompare_opp 53 1024
             (Bconv 24 128 53 1024 __ __ Float.of_single_nan mode_NE x) y (fun _ => pseudo_nan64) (fun _ => pseudo_nan64)).
  rewrite (Bcompare_opp 24 128
            x
            (Bconv 53 1024 24 128 __ __ Float.to_single_nan mode_DN y)
            (fun _ => pseudo_nan32) (fun _ => pseudo_nan32)).
  repeat rewrite cmp_le_expand by (assumption || lia).
  repeat rewrite option_map_CompOpp_invol.
  erewrite Bopp_Bconv_DN by (constructor || lia || assumption).
  erewrite <- Bcompare_Bconv_widen_ge by lia.
  erewrite Bopp_Bconv_NE by (constructor || lia || assumption).
  reflexivity.

  Unshelve.
  exact (fun _ => pseudo_nan32).
  exact (fun _ => pseudo_nan64).
Qed.

Lemma cmp_gt_expand: forall cmp,
    match cmp 
  with
  | Some Gt => true
  | _ => false
    end = negb (cmp_ge_nan true (option_map CompOpp cmp)).
Proof.
  destruct cmp. 2: reflexivity. destruct c; reflexivity.
Qed.

Theorem gt_single_float: forall x y,
    Float.cmp Cgt (Float.of_single x) y =
      Float32.cmp Cgt x (Float.to_single_DN y).
Proof.
  intros. unfold Float.of_single, Float.to_single_DN. simpl.
  unfold Float.compare, Float32.compare.
  destruct (is_nan _ _ y || is_nan _ _ x) eqn:NAN.
  ...
  rewrite (Bcompare_opp 24 128
            ...
            (Bconv 53 1024 24 128 __ __ Float.to_single_nan mode_DN y)
            (fun _ => pseudo_nan32) (fun _ => pseudo_nan32)).
  repeat rewrite cmp_gt_expand by (assumption || lia).
  repeat rewrite option_map_CompOpp_invol.
  erewrite Bopp_Bconv_DN by (constructor || lia || assumption).
  erewrite <- Bcompare_Bconv_widen_ge by lia.
  erewrite Bopp_Bconv_NE by (constructor || lia || assumption).
...
Definition double_representable_as_single y :=
  (Float.of_single (Float.to_single y)) = y.

Definition double_representable_as_single_dec y :
  { double_representable_as_single y } +
    { ~(double_representable_as_single y) } :=
  Beq_dec _ _ (Float.of_single (Float.to_single y)) y.
  
Theorem cmp_single_float_exact:
  forall op x y
    (EQ: double_representable_as_single y),
    Float.cmp op (Float.of_single x) y = Float32.cmp op x (Float.to_single y).
Proof.
  intros.
  unfold Float.cmp, Float.compare, Float.of_single.
  erewrite Bcompare_Bconv_widen_eq1. 4: exact EQ. 2,3: lia.
  reflexivity.
Qed.
  
Theorem eq_single_float_not_exact:
  forall x y
    (NEQ: ~(double_representable_as_single y)),
    (Float.cmp Ceq (Float.of_single x) y) = false.
Proof.
  intros.
  unfold Float.cmp, Float.compare, Float.of_single. simpl.
  assert (Bcompare 53 1024
            (Bconv 24 128 53 1024 __ __ Float.of_single_nan mode_NE x) y <> Some Eq).
  { eapply Bcompare_Bconv_widen_eq3.
    lia. lia. exact NEQ. }
  destruct Bcompare. 2: reflexivity.
  destruct c; congruence.
Qed.
  
Theorem ne_single_float_not_exact:
  forall x y
    (NEQ: ~(double_representable_as_single y)),
    (Float.cmp Cne (Float.of_single x) y) = true.
Proof.
  intros.
  unfold Float.cmp, Float.compare, Float.of_single. simpl.
  assert (Bcompare 53 1024
