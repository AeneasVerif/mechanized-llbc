...
Fixpoint Z_reverse_bits_nat (n: nat) (x: Z) (acc: Z) : Z :=
  match n with
  | O => acc
  | S m => Z_reverse_bits_nat m (Z.div2 x) (Zshiftin (Z.odd x) acc)
  end.

Definition Z_reverse_bits (n: Z) (x: Z) : Z :=
  Z_reverse_bits_nat (Z.to_nat n) x 0.

Lemma Z_reverse_bits_nat_spec:
  forall (m: nat) x acc i, 0 <= i ->
  Z.testbit (Z_reverse_bits_nat m x acc) i =
    if zlt i (Z.of_nat m)
    then Z.testbit x (Z.of_nat m - 1 - i)
    else Z.testbit acc (i - Z.of_nat m).
Proof.
  induction m; intros.
- simpl. rewrite zlt_false by lia. f_equal. lia.
- cbn [Z_reverse_bits_nat]. rewrite IHm by lia. rewrite Nat2Z.inj_succ.
  destruct (zlt i (Z.of_nat m)).
  + rewrite zlt_true by lia.
    rewrite (Ztestbit_eq (Z.succ (Z.of_nat m) - 1 - i) x) by lia.
    rewrite zeq_false by lia.
    f_equal. lia.
  + rewrite Ztestbit_shiftin by lia.
    destruct (zeq (i - Z.of_nat m) 0).
    * rewrite zlt_true by lia.
      replace (Z.succ (Z.of_nat m) - 1 - i) with 0 by lia.
      rewrite Ztestbit_base. auto.
    * rewrite zlt_false by lia.
      f_equal. lia.
Qed.

Lemma Z_reverse_bits_spec:
  forall n x i, 0 <= i -> 0 <= n ->
  Z.testbit (Z_reverse_bits n x) i =
    if zlt i n then Z.testbit x (n - 1 - i) else false.
Proof.
  intros. unfold Z_reverse_bits.
  rewrite Z_reverse_bits_nat_spec by auto.
  rewrite Z2Nat.id by auto.
  destruct (zlt i n); auto.
  rewrite Ztestbit_0. auto.
Qed.

Lemma Z_reverse_bits_range:
  forall n x, 0 <= n -> 0 <= x -> 0 <= Z_reverse_bits n x < two_p n.
Proof.
  intros.
  assert (E: Z_reverse_bits n x = Z_reverse_bits n x mod two_p n).
  { apply equal_same_bits; intros.
    rewrite Ztestbit_mod_two_p by auto.
    destruct (zlt i n); auto.
    rewrite Z_reverse_bits_spec by lia.
    rewrite zlt_false by lia. auto. }
  rewrite E. apply Z_mod_lt. apply two_p_gt_ZERO. lia.
Qed.
